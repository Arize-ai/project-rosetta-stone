...
class SearchProductsTool extends Tool<StringToolOutput> {
  ...
  public readonly emitter: ToolEmitter<Input, StringToolOutput> = Emitter.root.child({
    namespace: ["tool", "search_products"],
    creator: this,
  });
  ...
  protected async _run(input: Input): Promise<StringToolOutput> {
    ...
    if (input.query) {
      ...
      else if (conditions.length > 1) Object.assign(where, { $and: conditions });
      ...
      if (vectorIds && vectorIds.length > 0) {
        ...
        const idOrder = new Map(vectorIds.map((id, i) => [id, i] as const));
        filtered = products
        ...
        return new StringToolOutput(
          JSON.stringify({ results, totalFound: filtered.length }),
        );
      }
    ...
    return new StringToolOutput(JSON.stringify({ results, totalFound: filtered.length }));
  }
}
