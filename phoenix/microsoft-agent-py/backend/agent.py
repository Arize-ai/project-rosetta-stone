...
# Per-user threads: keyed by user_id for conversation memory
_threads: dict[str, AgentThread] = {}
# Track expected turn count per thread so we can detect resets
...
async def stream_agent(messages: list[dict], user_id: str) -> AsyncIterator[str]:
    ...
    if user_id not in _threads or assistant_turns < existing_turns:
        # Create a fresh thread (new conversation or conversation reset)
        _threads[user_id] = agent.get_new_thread()
    ...
    try:
        async for chunk in agent.run_stream(last_message, thread=thread):
    ...
    # Update thread turn count after successful response
