# Wonder Toys — LlamaIndex Python (Phoenix Instrumented)

This is the LlamaIndex Python variant of the Wonder Toys shopping agent, instrumented with Arize Phoenix for observability.
...
The LlamaIndex instrumentor does not create a top-level agent span with `input.value`/`output.value` attributes that Phoenix can display. It also doesn't create trace boundaries between requests — without intervention, all spans from all requests merge into a single trace.
...
try:
    ...
    #    Phoenix shows the user query and agent response at the trace level.
...
The `register()` call uses `batch=True` to avoid the `SimpleSpanProcessor` default, which does synchronous HTTP exports that can drop spans in async contexts.
...
| `evals/` | **New** — synthetic request harness + eval runner |
