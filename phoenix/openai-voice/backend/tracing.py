...
WebSocket usage (the `openinference-instrumentation-openai-agents` package
covers the *Agents SDK* runtime, not direct WS use), so we register a
tracer provider here (via `arize-phoenix-otel`) and hand-roll spans in the
voice agent following the same Arize audio recipe used by the AX tier:

  https://arize.com/docs/ax/cookbooks/evaluation/tracing-and-evaluating-audio

The cookbook is hosted under the AX docs but the OpenInference attributes
it uses (`input.audio.*`, `output.audio.*`, `llm.tools.{i}.tool.*`) are
the same ones Phoenix consumes — only the tracer-provider registration
differs between backends.

For the text-mode fallback we hand-roll a single `chat_completion` span per
turn, keeping this tier dependency-light.
