...
async def run_voice_session(browser_ws: WebSocket, user_id: str) -> None:
    ...
    try:
        ...
        # Long-running servers don't auto-flush the Agents SDK trace
        # processors; flush on session end so the turn spans reach the OTel
        # BatchSpanProcessor (which then flushes to Phoenix).
        flush_traces()
