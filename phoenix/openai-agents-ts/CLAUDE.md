...
- **`src/ai/tracing.ts`** — calls `register({ projectName, url, apiKey, spanProcessors: [...] })` from `@arizeai/phoenix-otel`, passing a local `OpenInferenceFilteredBatchSpanProcessor` (subclass of `BatchSpanProcessor` from `@opentelemetry/sdk-trace-base`, defined in `src/ai/oi-filter-processor.ts`). Then `new OpenAIAgentsInstrumentation({ tracerProvider }).manuallyInstrument(agents)` on the imported `@openai/agents` namespace.
- **`src/ai/oi-filter-processor.ts`** — ~15-line subclass that drops any span whose attributes don't include `SemanticConventions.OPENINFERENCE_SPAN_KIND`. Same predicate the Vercel package's `isOpenInferenceSpan` uses, inlined locally so the OpenAI Agents tier doesn't pull in `@arizeai/openinference-vercel`.

The custom processor is **load-bearing**: Next.js's built-in OTel auto-instrumentation otherwise pipes its own HTTP / fetch / page-render spans through whatever global provider is registered, polluting the Phoenix project alongside the agent spans. Filtering by `openinference.span.kind` leaves only `AGENT` / `LLM` / `TOOL` / `GUARDRAIL` / `CHAIN` spans.
