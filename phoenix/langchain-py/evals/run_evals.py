...
def extract_root_data(span) -> tuple[str, str]:
    ...
    LangChain OpenInference spans store input.value as a JSON string like:
      {"messages": [{"content": "...", "additional_kwargs": {}, "type": "human", ...}, ...]}
    and output.value similarly with the full message history including the AI response.
    """
    ...
    if not user_query:
        try:
            ...
            # LangChain messages use "type" field: "human", "ai", "system"
            # or standard "role" field
    ...
    try:
        ...
        if isinstance(parsed, dict):
            # LangChain output has {"messages": [...]} — find last AI message
            messages = parsed.get("messages", [])
            for msg in reversed(messages):
                if not isinstance(msg, dict):
                    continue
            ...
            if not agent_response:
                agent_response = parsed.get("text", "") or parsed.get("content", "") or ""
...
def main():
    print("Wonder Toys — Eval Runner (LangChain Python)")
