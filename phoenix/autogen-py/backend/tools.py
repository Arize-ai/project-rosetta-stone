"""Wonder Toys tool functions for AutoGen AgentChat.

AutoGen's :class:`FunctionTool` introspects ``Annotated`` parameter metadata
expecting a plain ``str`` description (``Annotated[T, "what this is"]``).
The Pydantic-style ``Annotated[Optional[T], Field(description=...)]`` that
the other tiers use blows up with::

    ValueError: Invalid description annotation=NoneType required=True
                description='...' for parameter X, should be a string.

So this tier uses plain-string Annotated descriptions. The function bodies
and return shapes are otherwise identical to the other tiers.
"""
...
def search_products(
    query: Annotated[
        Optional[str],
        "Free-text search query to match against product names and descriptions",
    ] = None,
    keywords: Annotated[
        Optional[list[str]],
        "Specific keywords to match against product keyword tags",
    ] = None,
    min_age: Annotated[
        Optional[int],
        "Minimum age in years for the target child",
    ] = None,
    max_age: Annotated[
        Optional[int],
        "Maximum age in years for the target child",
    ] = None,
    category: Annotated[
        Optional[str],
...
def purchase_product(
    items: Annotated[list[PurchaseItem], "List of products and quantities to purchase"],
...
def check_order_status(
    order_id: Annotated[
        Optional[str],
        "Specific order ID to look up (e.g. 'A1B2C3D4')",
    ] = None,
    product_search: Annotated[
        Optional[str],
        "Search term to find orders by product name (e.g. 'puzzle' or 'train')",
    ] = None,
) -> dict:
