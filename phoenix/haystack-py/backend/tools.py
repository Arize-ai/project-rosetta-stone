...
# ---------------------------------------------------------------------------
#
# Haystack's @tool decorator generates JSON schemas from type hints via
# pydantic create_model. Plain `list[dict]` works reliably; nested BaseModel
# parameters are not officially supported. Each item must be a dict with
# `product_id` (str) and `quantity` (int).
...
def purchase_product(
    items: Annotated[
        list[dict],
        "List of items to purchase. Each item is an object with 'product_id' (string, e.g. 'toy-001') and 'quantity' (integer >= 1).",
    ],
