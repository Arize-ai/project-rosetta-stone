...
echo "Starting Java backend on port 18002..."
java -jar "$BOOT_JAR" &
BACKEND_PID=$!
...
for i in $(seq 1 30); do
  if curl -sf "http://localhost:18002/products/featured" > /dev/null 2>&1; then
...
echo "  - Java backend: http://localhost:18002"
echo "  - (Spring AI tier — uses 18002 to coexist with the LangChain4j tier on 8001)"
