...
def search_products(
    ...
    Returns:
        A dict with a `results` list of matching products and a `totalFound` count.
    """
...
def get_product_detail(product_id: str) -> dict:
    ...
    Returns:
        A dict with a `found` boolean and (if found) a `product` dict with full details.
    """
...
def purchase_product(
    ...
    Args:
        items: List of products to purchase. Each item is a dict with keys `product_id` (str, e.g. 'toy-001') and `quantity` (int, minimum 1).
    ...
    Returns:
        A dict with `success`, `orderId`, `total`, and `items` on success, or `success: False` plus an `error` field on failure.
    """
...
def check_order_status(
    ...
    Returns:
        A dict with `found` and `orders` (a list of matching orders with item, total, status, and shipping details).
    """
    user_id = current_user_id.get()
...
def cancel_order_tool(order_id: str) -> dict:
    ...
    Returns:
        A dict describing the cancellation outcome.
    """
    user_id = current_user_id.get()
