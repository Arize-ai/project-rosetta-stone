...
echo ""
echo "Starting Python backend on port 8031..."
cd "$APP_DIR"
uvicorn backend.main:app --host 0.0.0.0 --port 8031 --reload &
BACKEND_PID=$!
PIDS_TO_KILL+=("$BACKEND_PID")

# Wait for backend to be ready
for i in $(seq 1 15); do
  if curl -sf "http://localhost:8031/products/featured" > /dev/null 2>&1; then
...
echo ""
echo "Starting Next.js dev server on port 3030..."
cd "$APP_DIR"
npx next dev -p 3030 &
NEXT_PID=$!
...
echo "  - Python backend: http://localhost:8031"
