...
public class ShoppingAgent {
  ...
  public ShoppingAgent(
      StreamingChatModel model, WonderToysTools tools, LangChain4jInstrumentor instrumentor) {
    this.model = model;
    this.tools = tools;
  ...
  public Assistant build(List<Map<String, String>> history, String userId) {
    ...
    return AiServices.builder(Assistant.class)
        ...
        .chatMemory(memory)
        // OpenInference AiService listeners emit the AGENT / CHAIN / TOOL spans around
        // each agent run + tool invocation. Without these the model spans land but the
        // surrounding agent context is missing.
