...
echo "Starting Java backend on port 18005..."
java -jar "$BOOT_JAR" &
BACKEND_PID=$!
...
for i in $(seq 1 30); do
  if curl -sf "http://localhost:18005/products/featured" > /dev/null 2>&1; then
...
echo "  - Java backend: http://localhost:18005"
echo "  - (Arconia tier — uses 18005 to coexist with the other Java tiers)"
