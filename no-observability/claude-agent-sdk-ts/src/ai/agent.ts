...
export async function* streamAgent(
  ...
  const q = query({ prompt: lastMessage, options: buildOptions(resumeSessionId) });

  for await (const message of q) {
    // Every SDK message carries the session id; keep the latest so the next
    // turn can resume this conversation.
