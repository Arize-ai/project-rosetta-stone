...
- Both tiers add an `OpenInferenceFilteredBatchSpanProcessor` (a ~15-line subclass of `BatchSpanProcessor` in `src/ai/oi-filter-processor.ts`) so HTTP / fetch / page-render spans emitted by Next.js's auto-OTel don't pollute the project bucket. Phoenix calls `@arizeai/phoenix-otel`'s `register()` and passes the filter via `spanProcessors`; AX builds a `NodeTracerProvider` by hand with the same filter and additionally skips `provider.register()` plus calls `getTracerProvider()?.forceFlush()` after each chat turn.
