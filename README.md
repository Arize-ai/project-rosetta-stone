...
│   └── langchain-js/            LangChain.js / LangGraph
...
| Framework | Agent library | LLM client | Streaming API |
|-----------|---------------|------------|---------------|
| **Mastra** | `@mastra/core` Agent | `@ai-sdk/anthropic` (Vercel AI SDK) | `stream.fullStream` |
| **LangChain.js** | `@langchain/langgraph` ReAct agent | `@langchain/anthropic` | `streamEvents` (v2) |
...
| **phoenix** | [Arize Phoenix Cloud](https://phoenix.arize.com) — open-source observability via `@mastra/arize` (Mastra) or `@arizeai/phoenix-otel` (LangChain.js) |
...
- **Instrumentation complexity**: How many lines of code and config changes does each observability platform require?
- **Platform capabilities**: How do Phoenix Cloud and Arize AX differ in their eval workflows (programmatic vs. UI-driven)?
