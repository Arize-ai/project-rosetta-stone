...
- `env.example` — observability environment variables
- `evals/` — **New directory** in observability tiers (synthetic requests + eval harness). AX Agent-as-a-Judge scoring examples live in repo-root `evals/aaaj/`. To attach them to traces, use the `rosetta-aaaj` skill (default `PROJECT_DIR` is `ax/langchain-py`; any `ax/<framework>` works after traces exist); do not use `ax evaluators create` for harness evals.
